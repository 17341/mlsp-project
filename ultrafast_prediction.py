...
def get_prediction(text, ultrafast_model, ultrafast_tokenizer):
    text += " <mask> ."
    input_ids, mask_idx = encode(ultrafast_tokenizer, text)
    with torch.no_grad():
        predict = ultrafast_model(input_ids)['outputs']
...
def load_ultrafast():
    ...
    ultrafast_model = AutoModelForMaskedLM.from_pretrained("pbelcak/UltraFastBERT-1x11-long").eval()
    return ultrafast_model, ultrafast_tokenizer